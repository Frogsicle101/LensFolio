...
public class WeblinkRegex {
    ...
    private static final String PCT = "(%[0-9a-f][0-9a-f])";
    private static final String RESERVED = "[;/?:@&=+$,]";
    private static final String UNRESERVED = "[a-zA-Z0-9-_\\.!~*'()]";
    private static final String URIC = RESERVED + UNRESERVED + PCT;

    private static final String PATH_CHARS = "a-zA-Z0-9-\\._~!$&'()*+,;=:@" + PCT;
    private static final String QUERY_CHARS = "[%s]=[%<s](&[%<s]=[%<s])*".formatted(URIC);
    ...
    private static final String QUERY = "(\\?[%s]+)?".formatted(QUERY_CHARS);
    private static final String FRAGMENT = "(#[%s]*)?".formatted(URIC);

    private static final Pattern WEBLINK = Pattern.compile(PROTOCOL +
            DOMAIN +
            PORT +
            PATH +
            QUERY +
