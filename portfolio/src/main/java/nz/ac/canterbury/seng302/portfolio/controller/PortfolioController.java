...
/** Controller class for the portfolio page */
@Controller
...
public class PortfolioController {
  ...
  /**
   * Autowired constructor for PortfolioController to automatically inject the requried beans
  ...
  public PortfolioController(
          ...
          WebLinkRepository webLinkRepository, RegexService regexService)
      throws InvalidNameException, MalformedURLException {
  ...
  public ModelAndView getPortfolio(
    ...
    try {
      logger.info("GET REQUEST /portfolio - retrieving all the information for the portfolio page");
      ...
      List<Milestone> milestoneList =
          milestoneRepository.findAllByProjectIdOrderByEndDate(projectId);
      int nextMilestoneNumber =
          milestoneRepository.countMilestoneByProjectId(projectId).intValue() + 1;
      LocalDate defaultOccasionDate =
          project.getStartDate(); // Today is in a sprint, the start of th project otherwise
    ...
    try {
      ...
      Project project =
          projectRepository
              ...
              .orElseThrow(
                  () ->
                      new EntityNotFoundException("Event with id " + projectId + "was not found"));
    ...
    try {
      ...
      if (projectStart.isBefore(project.getStartDate().minusYears(1))) {
        return new ResponseEntity<>(
            "Project cannot start more than a year before its original date",
            HttpStatus.BAD_REQUEST);
      }

      List<Sprint> sprintListEndDates = sprintRepository.getAllByProjectOrderByEndDateDesc(project);
      List<Sprint> sprintListStartDates =
          sprintRepository.getAllByProjectOrderByStartDateAsc(project);
      if (!sprintListEndDates.isEmpty()) {
        Sprint sprint = sprintListEndDates.get(0);
        if (sprint.getEndDate().isAfter(projectEnd)) {
          String errorMessage =
              "Could not change project dates.  New project end date of "
                  + projectEnd
                  + " is before the sprint: "
                  + sprint.getName()
                  + " ends: "
        ...
        if (sprint.getStartDate().isBefore(projectStart)) {
          String errorMessage =
              "Could not change project dates. New project start date of: "
                  + projectStart
                  + " is after the sprint: "
                  + sprint.getName()
                  + " starts: "
  ...
  /**
   * Checks that the project request is valid, contains all the right things, and dates are correct
   * and in order.
   *
  ...
  private ResponseEntity<Object> checkProjectRequest(ProjectRequest projectRequest) {
    try {
      ...
      if (!regexPatterns.getTitleRegex().matcher(projectName).matches()) {
        return new ResponseEntity<>(
            "Project Name contains characters outside of a-z 0-9", HttpStatus.BAD_REQUEST);
      }
      if (!regexPatterns.getDescriptionRegex().matcher(projectDescription).matches()) {
        return new ResponseEntity<>(
            "Project description contains illegal characters", HttpStatus.BAD_REQUEST);
      }
  ...
  public ResponseEntity<Object> addSprint(@RequestParam(value = "projectId") Long projectId) {
    try {
      ...
      // If start date of sprint is after the project end date then send a message to the user
      // informing them
      ...
      if (startDate.isAfter(project.getEndDate())) {
        ...
        return new ResponseEntity<>(
            "No more room to add sprints within project dates!", HttpStatus.BAD_REQUEST);
      } else {
        // Check that if the end date (startDate.plus(3)weeks) is after project end date, then set
        // the end date
  ...
  private HashMap<String, LocalDate> checkNeighbourDatesForSprint(Project project, Sprint sprint) {
    // Gets a list of all sprints that belong to the project and orders them by start date: earliest
    // to latest
    ...
    if (indexOfNextSprint < sprintList.size() - 1) {
      ...
      // Adds an object to the view that limits the calendar to dates before the next sprints
      // starts.
  ...
  private ResponseEntity<Object> checkSprintRequest(SprintRequest sprintRequest) {
    try {
      ...
      regexService.checkInput(RegexPattern.GENERAL_UNICODE, sprintDescription, 1, 500, "Sprint description");
      regexService.checkInput(RegexPattern.HEX_COLOUR, sprintColour, 0, 50, "Hex colour");
      ...
      logger.warn("Sprint failed to meet requirement: Date(s) is in incorrect format, Start Date: {}, End Date: {}", sprintRequest.getSprintEndDate(), sprintRequest.getSprintEndDate());
      return new ResponseEntity<>("Date(s) is in incorrect format", HttpStatus.BAD_REQUEST);
    }
    catch (CheckException exception) {
      logger.warn("Sprint failed to meet requirement, {}", exception.getMessage());
  ...
  public void createDefaultEvents(Project project) throws InvalidNameException {
    ...
    Event event1 =
        new Event(
            project,
            "Term Break",
            LocalDateTime.parse("2022-04-11T08:00:00"),
    ...
    Event event2 =
        new Event(
            ...
            LocalDateTime.parse("2022-04-10T17:00:00"),
    ...
    Event event3 =
        new Event(
            ...
            LocalDateTime.parse("2022-05-18T15:00:00"),
    ...
    Event event4 =
        new Event(
            project,
            "Semester 2",
            LocalDateTime.parse("2022-07-18T15:00:00"),
            LocalDate.parse("2022-09-30"),
            LocalTime.now(),
            6);
  ...
  public void createDefaultDeadlines(Project project) {
    try {
      ...
      Deadline deadline4 =
          new Deadline(
              project,
              "NZ On Air Scripted General Audiences",
  ...
  public void createDefaultMilestones(Project project) throws InvalidNameException {
    Milestone milestone1 =
        new Milestone(project, "Finished the project!", LocalDate.parse("2022-05-01"), 1);
    Milestone milestone2 =
        new Milestone(project, "Lost all the money", LocalDate.parse("2022-06-01"), 2);
    Milestone milestone3 =
        new Milestone(project, "Wow look at that flying dog", LocalDate.parse("2022-07-01"), 3);
  ...
  public void createDefaultRepos() {
    GitRepository repo1 =
        new GitRepository(3, 13661, "Team 100's git Repository", "szMkVx_xM39gB5yRxSmL");
    gitRepoRepository.save(repo1);
    GitRepository repo2 =
        new GitRepository(4, 13737, "Team 200's git Repository", "ixgv4UTo--zGZ5Km1rQ");
  ...
  public void createDefaultEvidence() throws MalformedURLException {
    ...
    Evidence evidence1 =
        new Evidence(
            ...
            "Created test Data",
            LocalDate.now(),
            "Created a selection of default evidence objects for testing");
    Evidence evidence2 =
        new Evidence(9, "making more evidence", LocalDate.now(), "Description of another one");
    Evidence evidence3 =
        new Evidence(
            9,
            "Writing Long Descriptions",
            LocalDate.now(),
            "A really long Description. A really long Description. A really long Description. A really long Description. A really long Description. A really long Description. A really long Description. A really long Description. A really long Description. ");
    Evidence evidence4 =
        new Evidence(
            9,
            "No Skill Evidence",
