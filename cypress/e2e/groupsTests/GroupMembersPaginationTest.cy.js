context("Test group pagination", () => {
    ...
    it('Check amount of group members to display changes', () => {
        cy.get("#membersPerPageSelect").contains('[class="page-link"]', "1").click()
        cy.get("#membersPerPageSelect").get(".scrollableGroupOverview").scrollTo("top")
        cy.get("#membersPerPageSelect").get("#groupDisplayAmountSelection").select("10")
        cy.get("#membersPerPageSelect").get(".group ").should("have.length", 10)
        cy.get("#membersPerPageSelect").get(".scrollableGroupOverview").scrollTo("top")
        cy.get("#membersPerPageSelect").get("#groupDisplayAmountSelection").select("20").wait(1000)
        cy.get("#membersPerPageSelect").contains('[class="page-link"]', "1").click()
        cy.get("#membersPerPageSelect").get(".group ").should("have.length", 20)
        cy.get("#membersPerPageSelect").get("#groupDisplayAmountSelection").select("All")
        cy.get("#membersPerPageSelect").get(".group ").should("have.length.at.least", 21)
    })
